...
private:
    ...
    atomic<bool> m_stop;
    map<thread::id, thread> m_workers; // 浠诲姟闃熷垪
                                           ...
                                           m_minThreadNumber(min), m_stop(false), m_exitThreadNumber(0)
{
    ...
    cout << "绾跨▼鏁伴噺: " << m_curThreadNumber << endl;
...
ThreadPool::~ThreadPool()
{
    ...
    for (auto &it : m_workers)
    {
        ...
        if (t.joinable())
        {
            cout << "******** 绾跨▼ " << t.get_id() << " 灏嗚閫€鍑轰簡..." << endl;
            t.join();
        }
    }
...
void ThreadPool::manager()
{
    while (!m_stop.load())
    {
        ...
        {
            thread t(&ThreadPool::worker, this);
            cout << "+++++++++++++++ 娣诲姞浜嗕竴涓嚎绋�, id: " << t.get_id() << endl;
            m_workers.insert(make_pair(t.get_id(), move(t)));
            m_curThreadNumber++;
            m_idlThreadNumber++;
        }
    ...
    while (!m_stop.load())
    {
        function<void(void)> task = nullptr;
        {
            ...
            while (!m_stop.load() && m_taskQueue.empty())
            {
                ...
                {
                    cout << "----------------- 绾跨▼浠诲姟缁撴潫, ID: " << this_thread::get_id() << endl;
                    m_exitThreadNumber--;
                    m_curThreadNumber--;
                    unique_lock<mutex> lck(m_taskMutex);
